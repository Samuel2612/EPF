...
from ecf_tensor_einsum_filter import A_tensor_frequency_filtered
from ecf_tensor_einsum_filter2 import A_tensor_no_sign_blowup2
...
def tt_svd(tensor: np.ndarray, eps):
    T     = tensor.copy()
    dims  = T.shape
    d     = len(dims)
...
if __name__ == "__main__":
    ...
    # A_2 = build_cos_tensor(Ks, a, b, mu, Sigma)
    # print("Tensor shape:", A_2.shape)
    ...
    for i in range(2):
        for N in Ns:
            ...
            # A = A_tensor_no_sign_blowup2(X, a, b, s, Ks)
            if i == 0:
                A_1 = build_cos_coefficients(X, a, b, Ks)
            else:
                A_1 = A_tensor_no_sign_blowup(X, a, b, s, Ks)
        
            print("\nCompressing with TT-SVD ...")
            t0 = time.time()
            cores, ranks = tt_svd(A_1, eps=1e-12)
            ...
            xs    = rng.normal(mu[1:], std[1:], size=(100,1))
            ...
            print(f"\nN = {N}, type {i}:")
            print("Conditional raw-moment errors (TT vs. exact):")
            for p in (1,2,3,4):
                arr = np.array(errs[p])
