...
# ======================================================================================
# Original calibrator + results container
# ======================================================================================
...
class MVTCalibrator:
    def __init__(
        ...
        *,
        ...
        self.df[self._ts] = pd.to_datetime(self.df[self._ts], errors="coerce", dayfirst=parse_dayfirst)
        ...
        if tz is not None and self.df[self._ts].dt.tz is not None:
        ...
        if delivery_hour_col is not None:
            ...
            self.df[ds_col] = pd.to_datetime(self.df[ds_col], errors="coerce", dayfirst=parse_dayfirst)
            if self.df[ds_col].isna().any():
                raise ValueError(f"Column '{ds_col}' contains non-parsable delivery_start timestamps.")
            if tz is not None and self.df[ds_col].dt.tz is not None:
...
class MVTTrainedModel:
    ...
    def _row(self, date, hour: int, which_hour_before: int) -> pd.Series:
        m = (
            (self.params["date"] == pd.to_datetime(date).date()) &
            (self.params["hour"].astype(int) == int(hour)) &
            (self.params["which_hour_before"].astype(int) == int(which_hour_before))
        )
        if not m.any():
            raise KeyError(f"No trained params for date={date}, hour={hour}, wh={which_hour_before}")
...
# ======================================================================================
        ...
        # --- Prefilter to only relevant columns and time range for speed ---
        ts_col = self.cal._ts
        dh_col = self.cal._dh
        ds_col = self.cal._ds  # may be None

        needed = {ts_col, dh_col, self.cfg["main_feature_col"]}
        # include additional features (after log1p mapping in calibrator)
        needed.update(self.cal._other_features.values())
        if ds_col is not None:
            needed.add(ds_col)

        # Determine time bounds: start_day - lookback buffer; add 1 day cushion
        lb = int(self.cfg.get("lookback_days", 0)) + 1
        ...
        keep_cols = [c for c in sorted(needed) if c in self.cal.df.columns]
        self.cal.df = (
            self.cal.df.loc[m, keep_cols]
...
class MVTCalibrator:
    ...
    if delivery_hour_col is not None:
        ...
        # EASY FIX: parse exactly the same fixed format as the source provides
        if not np.issubdtype(self.df[ds_col].dtype, np.datetime64):
            self.df[ds_col] = pd.to_datetime(
                self.df[ds_col].astype(str).str.strip(),
                format="%d/%m/%Y %H:%M:%S",   # <- exact "DD/MM/YYYY hh:mm:ss"
                errors="coerce"
            )
        ...
        if self.df[ds_col].isna().any():
            ...
            raise ValueError(
                f"Column '{ds_col}' contains non-parsable timestamps (expected DD/MM/YYYY hh:mm:ss). "
                f"Example bad indices: {bad}"
            )
    
        # no tz conversion (you said these are naive wall-clock timestamps)
        self.df["_delivery_hour"] = self.df[ds_col].dt.hour
        self._dh = "_delivery_hour"
        self._ds = ds_col
...
class MVT_forecasting_study:
    ...
    def __init__(
        ...
        *,
        ...
        self.start_day = start_date.normalize() if tzinfo is None else (start_date.tz_localize(tzinfo).normalize() if start_date.tzinfo is None else start_date.normalize())
        self.end_day   = end_date.normalize()   if tzinfo is None else (end_date.tz_localize(tzinfo).normalize() if end_date.tzinfo is None else end_date.normalize())
...
if __name__ == "__main__":
    # Load your data
    df_2021 = pd.read_csv("df_2021_MVT_symm.csv", parse_dates=["bin_timestamp", "delivery_start"])
    df_2022 = pd.read_csv("df_2022_MVT_symm.csv", parse_dates=["bin_timestamp", "delivery_start"])  # fixed filename
    ...
    # ----------------- PHASE 1: TRAIN -----------------
    trained = study.train()
    trained.save_parquet("mvt_trained_params_2021_2022.parquet")

    # (Later / in a new session) you could load with:
    # trained = MVTTrainedModel.load_parquet("mvt_trained_params_2021_2022.parquet")

    # Quick access example
    ex = trained.get_params(date="2021-06-15", hour=12, which_hour_before=3)
    print("Example params keys:", list(ex.keys()))
