...
def _pin_path(subs: str, K: Sequence[int], dtype: np.dtype) -> oe.contract.PathType:
    warm = [np.empty((4, k), dtype=dtype) for k in K]
    path, _ = oe.contract_path(subs, *warm, optimize="greedy")
    return path
...
def ecf_grid_einsum(
    ...
    *,
    ...
    for j in range(d):
        ...
        E_list.append(np.exp(1j * alpha * X[:, j, None] * k_vec, dtype=dtype))
        in_subs.append(f"n{letters[j]}")
    subs = ",".join(in_subs) + "->" + "".join(letters[: d])

    phi_hat = oe.contract(subs, *E_list, optimize=_pin_path(subs, K, dtype)) / N
    return phi_hat

################################################################################
#  2. Flood‑fill -> largest contiguous hyper‑volume                            #
################################################################################
...
################################################################################
#  3. Self‑consistent fastKDE spectral filter                                  #
################################################################################
...
################################################################################
#  4. Convenience: raw + filtered in one call                                  #
################################################################################
...
################################################################################
#  5. COS coefficients with filtered CF                                        #
################################################################################
