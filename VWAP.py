...
class LimitOrderBookProcessor:
    ...
    def _preprocess_data(self):
        ...
        self.data = self.data[~self.data['Delivery Start'].dt.hour.isin([2, 3])]
    ...
    def merge_res_forecasts(self, df, res_csv):
        ...
        res_df = res_df[(res_df['Date (CET)'].dt.year == 2021) & (res_df['Date (CET)'].dt.hour != 2) ]
        res_df.set_index("Date (CET)", inplace=True)
        res_df.index.name = "delivery_start"
        res_df = res_df[["Day Ahead Forecast"]]
    ...
    def prepare_gamlss_data(self, product_type='hourly', start_hours=3, 
                            wind_csv='dayahead-forecast-wind.csv',
                            solar_csv='dayahead-forecast-solar.csv'):
        # Get aggregated data
        ...
        agg_df['TTD_bins'] = (agg_df['delivery_start'] - agg_df['bin_timestamp']).dt.total_seconds() / 5*60.0
        agg_df['f_TTD'] = 1.0 / np.sqrt(1.0 + agg_df['TTD_bins'].clip(lower=0))
        ...
        merged_df = self.merge_res_forecasts(merged_df, wind_csv)
        merged_df = merged_df.rename(columns={'Day Ahead Forecast': 'wind_forecast'})
        merged_df = self.merge_res_forecasts(merged_df, solar_csv)
        merged_df = merged_df.rename(columns={'Day Ahead Forecast': 'solar_forecast'})
        ...
        merged_df = self.add_lagged_variables(merged_df, 
                                              {'vwap' : [1], 
                                               'vwap_changes': [1,2,3], 'alpha' : [1,2], 
